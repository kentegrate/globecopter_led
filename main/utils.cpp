...
void gpio_initialize_interrupt(int gpio_num, gpio_int_type_t intr_type, gpio_pull_mode_t pullup_type){
  ...
  io_conf.pin_bit_mask = (GPIO_SEL_34 | GPIO_SEL_35);
  io_conf.pull_up_en = static_cast<gpio_pullup_t>(1);
  io_conf.pull_down_en = static_cast<gpio_pulldown_t>(0);
  io_conf.mode = GPIO_MODE_INPUT;
  gpio_config(&io_conf);
  gpio_set_intr_type(static_cast<gpio_num_t>(34), intr_type);
  gpio_set_intr_type(static_cast<gpio_num_t>(35), intr_type);    
  //  
